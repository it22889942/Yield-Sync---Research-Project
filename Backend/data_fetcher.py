...
def download_harti_pdf(year: int, week: int, week_iso: int = None) -> bytes:
    """
    Download HARTI weekly bulletin PDF.
    Tries %W-based week number first, then ISO week number as fallback,
    so mismatches in HARTI's own numbering are handled automatically.
    Timeout is short (8s) so failed attempts don't stall the update loop.
    """
    # Build list of week numbers to try (deduplicated)
    candidates = [week]
    if week_iso is not None and week_iso != week:
        candidates.append(week_iso)

    for w in candidates:
        url = f"{HARTI_BASE_URL}/{year}/weekly/weekly_{w:02d}_{year}_Eng.pdf"
        print(f"📥 Downloading: {url.split('/')[-1]}")
        try:
            ...
            response.raise_for_status()
            print(f"   ✅ Downloaded week {w:02d}")
            return response.content
        except Exception as e:
            print(f"   ❌ Download failed (week {w:02d}): {e}")
...
def fetch_week(year: int, week: int, last_week_prices: pd.DataFrame = None,
               week_iso: int = None) -> pd.DataFrame:
    """
    Fetch combined price + weather data for a week.
    Uses last week's prices as fallback if PDF not available.
    ...
    Args:
        ...
        week     : Monday-based (%W) week number — consistent with strptime usage.
        week_iso : ISO week number (isocalendar) — used as secondary URL candidate.

    Returns:
        DataFrame with columns matching historical format
    """
    # week_start: derive directly from %W week number (consistent with how
    # week was computed — avoids the ISO vs %W off-by-one mismatch).
    week_start = datetime.strptime(f'{year}-W{week:02d}-1', '%Y-W%W-%w')

    print(f"\n{'='*50}")
    print(f"📅 Fetching Week {week} (%W), {year} ({week_start.date()})")
    print('='*50)

    # 1. Get prices — try primary (%W) week number, then ISO as fallback
    pdf_bytes = download_harti_pdf(year, week, week_iso=week_iso)
...
def update_this_week(data_path: str = None):
    ...
    today = datetime.now()
    monday = today - timedelta(days=today.weekday())  # snap to Monday
    year = monday.year
    week = int(monday.strftime('%W'))        # %W: Monday-based week number
    week_iso = monday.isocalendar()[1]       # ISO week: used as URL fallback

    data = fetch_week(year, week, week_iso=week_iso)
    if not data.empty:
        update_dataset(data, data_path)
...
def update_since_date(start_date: str, data_path: str = None):
    ...
    while current <= end:
        monday = current - timedelta(days=current.weekday())  # ensure Monday
        year = monday.year
        week = int(monday.strftime('%W'))      # %W: consistent with fetch_week
        week_iso = monday.isocalendar()[1]     # ISO: used as URL fallback

        data = fetch_week(year, week, last_prices, week_iso=week_iso)
...
def update_complete_weeks_only(start_date: str = None, data_path: str = None):
    ...
    # Snap start to Monday so week numbers are always aligned
