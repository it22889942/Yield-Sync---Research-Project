...
import os, re, math, joblib, numpy as np, pandas as pd, requests
from datetime import datetime, timezone
from flask import Blueprint, request, jsonify

# ---------------------- Blueprint ---------------------- #
crop_bp = Blueprint("crop_bp", __name__, url_prefix="/api/crop")

# ---------------------- Config ---------------------- #
ARTIFACT_PATH = os.environ.get("MODEL_PATH", os.path.join("models", "best_model.joblib"))
...
if not os.path.isfile(ARTIFACT_PATH):
    raise FileNotFoundError(
        ...
        "Put joblib inside Backend/models/ or set MODEL_PATH."
    )
...
def predict():
    try:
        ...
        for row in raw_rows:
            ...
            if nr.get("Rainfall", None) is None:
                lat = nr.get("latitude")
                lon = nr.get("longitude")
                if lat is not None and lon is not None:
                    nr["Rainfall"] = get_rainfall_mm(float(lat), float(lon), hours=int(row.get("hours", 24)))
