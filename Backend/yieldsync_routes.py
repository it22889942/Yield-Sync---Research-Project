...
def recommendation():
    ...
    days_since_harvest = _q("days_since_harvest", 0)
    transport_cost_per_kg = _q("transport_cost_per_kg")
    storage_cost_per_kg_day = _q("storage_cost_per_kg_day")
    fixed_cost_total = _q("fixed_cost_total")
    spoilage_rate = _q("spoilage_rate")
    ...
    try:
        ...
        days_since_harvest = int(days_since_harvest)

        if transport_cost_per_kg is not None:
            transport_cost_per_kg = float(transport_cost_per_kg)
        if storage_cost_per_kg_day is not None:
            storage_cost_per_kg_day = float(storage_cost_per_kg_day)
        if fixed_cost_total is not None:
            fixed_cost_total = float(fixed_cost_total)
        if spoilage_rate is not None:
            spoilage_rate = float(spoilage_rate)
    except Exception:
        return jsonify({
            "error": (
                "Invalid numeric input. Required: days_ahead(int), quantity_kg(number), "
                "days_since_harvest(int). Optional numeric fields: transport_cost_per_kg, "
                "storage_cost_per_kg_day, fixed_cost_total, spoilage_rate"
            )
        }), 400
    ...
    result = api.get_recommendation(
        ...
        storage_cost_per_kg_day=storage_cost_per_kg_day,
