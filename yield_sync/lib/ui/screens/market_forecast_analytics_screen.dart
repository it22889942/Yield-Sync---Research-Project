...
class _MarketForecastAnalyticsScreenState
  ...
  Future<void> _loadAll() async {
    ...
    try {
      ...
      for (final h in _horizons) {
        ...
        final resolvedChangePct = rec.containsKey("price_change_percent")
        ...
        final String bestTimingText = (rec["best_timing"] ?? "").toString();
  ...
  Widget build(BuildContext context) {
    ...
    final transportCost = _asDouble(costBreakdown["transport_cost_later"]) > 0
        ? _asDouble(costBreakdown["transport_cost_later"])
        : _asDouble(costBreakdown["transport_cost_now"]);
    final expenditure = transportCost +
    ...
    final expectedPrice = _asDouble(sel["expected_price"]);
    final bestTimingText = (sel["best_timing_text"] ?? "").toString().trim();
    final bestTimingDays = _asInt(sel["best_timing_days"]);
    final bestTimingLabel = shelfLife <= 1
        ? "Sell Now"
        : (bestTimingText.isNotEmpty
            ? bestTimingText
            : (bestTimingDays >= 0
                ? (bestTimingDays == 0
                    ? "Sell Now"
                    : "Hold for $bestTimingDays days")
                : (decision.toUpperCase().contains("SELL")
                    ? "Sell Now"
                    : "Hold for $selectedH days")));
                    ...
                    Container(
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          Text(
                            "Expected price: ${_fmt2(expectedPrice)} LKR/kg",
                            style: const TextStyle(
                              color: Colors.white,
                              fontWeight: FontWeight.w800,
                              height: 1.25,
                    ...
                    Row(
                      children: [
                        ...
                        Expanded(
                          child: _QuickStat(
                            title: "Revenue (Hold)",
                            value: profitAnalysis.isNotEmpty
                                ? _fmtCompactLkr(profitNow)
                                : "N/A",
                            icon: Icons.trending_up_rounded,
                            valueColor: profitNow >= 0
                                ? const Color(0xFF2FA36B)
                                : const Color(0xFFE05C5C),
                          ),
                        ),
                          ...
                          children: [
                            ...
                            if (costBreakdown["storage_cost_total"] != null)
                              _kv("Total Storage Cost",
                                  _fmtLkr(_asDouble(costBreakdown["storage_cost_total"]))),
                            if (costBreakdown["storage_cost_total"] != null)
                              const SizedBox(height: 10),
